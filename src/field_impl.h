...
/* Execute independent square-root addition chains in lockstep. Apple ARM64 has
 * enough integer multiply throughput to overlap the long dependency chains,
 * while a single secp256k1_fe_sqrt leaves that throughput underutilized. */
static int secp256k1_fe_sqrt_interleaved(
        secp256k1_fe r[HARBOR_SECP256K1_ARM64_SQRT_BATCH_WIDTH],
        const secp256k1_fe a[HARBOR_SECP256K1_ARM64_SQRT_BATCH_WIDTH]) {
    secp256k1_fe x2[HARBOR_SECP256K1_ARM64_SQRT_BATCH_WIDTH];
    secp256k1_fe x3[HARBOR_SECP256K1_ARM64_SQRT_BATCH_WIDTH];
    secp256k1_fe x6[HARBOR_SECP256K1_ARM64_SQRT_BATCH_WIDTH];
    secp256k1_fe x9[HARBOR_SECP256K1_ARM64_SQRT_BATCH_WIDTH];
    secp256k1_fe x11[HARBOR_SECP256K1_ARM64_SQRT_BATCH_WIDTH];
    secp256k1_fe x22[HARBOR_SECP256K1_ARM64_SQRT_BATCH_WIDTH];
    secp256k1_fe x44[HARBOR_SECP256K1_ARM64_SQRT_BATCH_WIDTH];
    secp256k1_fe x88[HARBOR_SECP256K1_ARM64_SQRT_BATCH_WIDTH];
    secp256k1_fe x176[HARBOR_SECP256K1_ARM64_SQRT_BATCH_WIDTH];
    secp256k1_fe x220[HARBOR_SECP256K1_ARM64_SQRT_BATCH_WIDTH];
    secp256k1_fe x223[HARBOR_SECP256K1_ARM64_SQRT_BATCH_WIDTH];
    secp256k1_fe t1[HARBOR_SECP256K1_ARM64_SQRT_BATCH_WIDTH];
    size_t i;
    int j;
    int ret = 1;
    ...
    for (j = 0; j < 3; ++j) {
        for (i = 0; i < HARBOR_SECP256K1_ARM64_SQRT_BATCH_WIDTH; ++i) secp256k1_fe_sqr(&x6[i], &x6[i]);
    }
    for (i = 0; i < HARBOR_SECP256K1_ARM64_SQRT_BATCH_WIDTH; ++i) {
    ...
    for (j = 0; j < 3; ++j) {
        for (i = 0; i < HARBOR_SECP256K1_ARM64_SQRT_BATCH_WIDTH; ++i) secp256k1_fe_sqr(&x9[i], &x9[i]);
    }
    for (i = 0; i < HARBOR_SECP256K1_ARM64_SQRT_BATCH_WIDTH; ++i) {
    ...
    for (j = 0; j < 2; ++j) {
        for (i = 0; i < HARBOR_SECP256K1_ARM64_SQRT_BATCH_WIDTH; ++i) secp256k1_fe_sqr(&x11[i], &x11[i]);
    }
    for (i = 0; i < HARBOR_SECP256K1_ARM64_SQRT_BATCH_WIDTH; ++i) {
    ...
    for (j = 0; j < 11; ++j) {
        for (i = 0; i < HARBOR_SECP256K1_ARM64_SQRT_BATCH_WIDTH; ++i) secp256k1_fe_sqr(&x22[i], &x22[i]);
    }
    for (i = 0; i < HARBOR_SECP256K1_ARM64_SQRT_BATCH_WIDTH; ++i) {
    ...
    for (j = 0; j < 22; ++j) {
        for (i = 0; i < HARBOR_SECP256K1_ARM64_SQRT_BATCH_WIDTH; ++i) secp256k1_fe_sqr(&x44[i], &x44[i]);
    }
    for (i = 0; i < HARBOR_SECP256K1_ARM64_SQRT_BATCH_WIDTH; ++i) {
    ...
    for (j = 0; j < 44; ++j) {
        for (i = 0; i < HARBOR_SECP256K1_ARM64_SQRT_BATCH_WIDTH; ++i) secp256k1_fe_sqr(&x88[i], &x88[i]);
    }
    for (i = 0; i < HARBOR_SECP256K1_ARM64_SQRT_BATCH_WIDTH; ++i) {
    ...
    for (j = 0; j < 88; ++j) {
        for (i = 0; i < HARBOR_SECP256K1_ARM64_SQRT_BATCH_WIDTH; ++i) secp256k1_fe_sqr(&x176[i], &x176[i]);
    }
    for (i = 0; i < HARBOR_SECP256K1_ARM64_SQRT_BATCH_WIDTH; ++i) {
    ...
    for (j = 0; j < 44; ++j) {
        for (i = 0; i < HARBOR_SECP256K1_ARM64_SQRT_BATCH_WIDTH; ++i) secp256k1_fe_sqr(&x220[i], &x220[i]);
    }
    for (i = 0; i < HARBOR_SECP256K1_ARM64_SQRT_BATCH_WIDTH; ++i) {
    ...
    for (j = 0; j < 3; ++j) {
        for (i = 0; i < HARBOR_SECP256K1_ARM64_SQRT_BATCH_WIDTH; ++i) secp256k1_fe_sqr(&x223[i], &x223[i]);
    }
    for (i = 0; i < HARBOR_SECP256K1_ARM64_SQRT_BATCH_WIDTH; ++i) {
    ...
    for (j = 0; j < 23; ++j) {
        for (i = 0; i < HARBOR_SECP256K1_ARM64_SQRT_BATCH_WIDTH; ++i) secp256k1_fe_sqr(&t1[i], &t1[i]);
    }
    for (i = 0; i < HARBOR_SECP256K1_ARM64_SQRT_BATCH_WIDTH; ++i) secp256k1_fe_mul(&t1[i], &t1[i], &x22[i]);
    for (j = 0; j < 6; ++j) {
        for (i = 0; i < HARBOR_SECP256K1_ARM64_SQRT_BATCH_WIDTH; ++i) secp256k1_fe_sqr(&t1[i], &t1[i]);
    }
    for (i = 0; i < HARBOR_SECP256K1_ARM64_SQRT_BATCH_WIDTH; ++i) {
